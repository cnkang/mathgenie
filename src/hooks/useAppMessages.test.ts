...
describe('useAppMessages', () => {
  ...
  test('should handle multiple showSuccessMessage calls', () => {
    ...
    // First message
    act(() => {
      result.current.showSuccessMessage('First message');
    });

    expect(result.current.successMessage).toBe('First message');

    // Second message before first timeout
    act(() => {
      vi.advanceTimersByTime(2000); // 2 seconds
      result.current.showSuccessMessage('Second message');
    });

    expect(result.current.successMessage).toBe('Second message');
  ...
  test('should not interfere with manual setSuccessMessage', () => {
    ...
    act(() => {
      result.current.showSuccessMessage('Auto message');
    });

    expect(result.current.successMessage).toBe('Auto message');

    // Manually set a different message
    act(() => {
      result.current.setSuccessMessage('Manual message');
    });

    expect(result.current.successMessage).toBe('Manual message');
